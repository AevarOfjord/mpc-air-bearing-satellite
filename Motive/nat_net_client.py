    ...
    def __create_command_socket( self ):
        ...
        if self.use_multicast :
            ...
            try:
                ...
                result = None
            except  socket.herror:  # type: ignore[unreachable]
                print("ERROR: command socket herror occurred")
                result = None
            except  socket.gaierror:  # type: ignore[unreachable]
                print("ERROR: command socket gaierror occurred")
                result = None
            except  socket.timeout:  # type: ignore[unreachable]
                print("ERROR: command socket timeout occurred. Server not responding")
                result = None
            # set to broadcast mode
            result.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)  # type: ignore[union-attr]
            # set timeout to allow for keep alive messages
            result.settimeout(2.0)  # type: ignore[union-attr]
        else:
            # Unicast case
            ...
            # set timeout to allow for keep alive messages
            result.settimeout(2.0)  # type: ignore[union-attr]
            result.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # type: ignore[union-attr]
    ...
    def __create_data_socket( self, port ):
        ...
        if self.use_multicast:
            ...
            if(self.multicast_address != "255.255.255.255"):
                result.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, socket.inet_aton(self.multicast_address) + socket.inet_aton(self.local_ip_address))  # type: ignore[union-attr]
...
#Unpack Mocap Data Functions
    ...
    def __unpack_labeled_marker_data( self, data, packet_size, major, minor):
        ...
        if( ( major == 2 and minor > 3 ) or major > 2 ):
            ...
            for lm_num in range( 0, labeled_marker_count ):
                ...
                labeled_marker = mocap_data.LabeledMarker(tmp_id,pos,size,param, residual)  # type: ignore[arg-type]
    ...
    def __unpack_mocap_data( self, data : bytes, packet_size, major, minor):
        mocap_frame_data = mocap_data.MoCapData()
        ...
        offset += rel_offset
        mocap_frame_data.set_prefix_data(frame_prefix_data)
        ...
        offset += rel_offset
        mocap_frame_data.set_marker_set_data(marker_set_data)
        ...
        offset += rel_offset
        mocap_frame_data.set_legacy_other_markers(legacy_other_markers)
        ...
        offset += rel_offset
        mocap_frame_data.set_rigid_body_data(rigid_body_data)
        ...
        offset += rel_offset
        mocap_frame_data.set_skeleton_data(skeleton_data)
        ...
        if (((major == 4) and (minor > 0)) or (major > 4)):
            ...
            offset += rel_offset
            mocap_frame_data.set_asset_data(asset_data)
        ...
        offset += rel_offset
        mocap_frame_data.set_labeled_marker_data(labeled_marker_data)
        ...
        offset += rel_offset
        mocap_frame_data.set_force_plate_data(force_plate_data)

        # Device Data
        rel_offset,device_data = self.__unpack_device_data(data[offset:], (packet_size - offset),major, minor)
        offset += rel_offset
        mocap_frame_data.set_device_data(device_data)
        ...
        offset += rel_offset
        mocap_frame_data.set_suffix_data(frame_suffix_data)
        ...
        return offset, mocap_frame_data
    ...
    def __unpack_force_plate_description(self, data, major, minor):
        ...
        if major >= 3:
            ...
            for i in range(0,12):
                ...
                cal_matrix_tmp[i] = copy.deepcopy(cal_matrix_row)  # type: ignore[call-overload,arg-type]
                offset += (12*4)
                ...
                if message_id == self.NAT_FRAMEOFDATA:
                    if print_level > 0:
                        if (message_id_dict[tmp_str] % print_level) == 0:
                            ...
                            print_level = 0
                message_id = self.__process_message( data , print_level)  # type: ignore[arg-type]
    ...
    def __process_message( self, data : bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA :
            ...
            offset += offset_tmp
            print("MoCap Frame: %d\n"%(mocap_data.prefix_data.frame_number))  # type: ignore[union-attr]
            ...
            offset_tmp, data_descs = self.__unpack_data_descriptions( data[offset:], packet_size, major, minor)  # type: ignore[misc]
            offset += offset_tmp
    ...
    def send_request( self, in_socket, command, command_str, address ):
        ...
        if command == self.NAT_CONNECT :
            data+=bytearray(command_str)  # type: ignore[arg-type]
        else:
            data += command_str.encode( 'utf-8' )  # type: ignore[attr-defined]
        data += b'\0'
    ...
    def shutdown(self):
        ...
        self.command_socket.close()  # type: ignore[union-attr]
        self.data_socket.close()  # type: ignore[union-attr]
        # attempt to join the threads back.
        self.command_thread.join()  # type: ignore[union-attr]
        self.data_thread.join()  # type: ignore[union-attr]
