...
class RigidBodyMarker:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        out_str += "%sSize    : %3.1f\n"%(out_tab_str, self.size[0])  # type: ignore[index]
        return out_str
...
class LabeledMarker:
    def __init__(self, new_id, pos, size=0.0, param = 0, residual=0.0):
        ...
        if str(type(size)) == "<class 'tuple'>":
            self.size=size[0]  # type: ignore[index]
...
def generate_rigid_body_marker_srand(marker_num=0, frame_num = 0):
    ...
    rigid_body_marker.error=random.random()  # type: ignore[assignment]
...
def generate_suffix_data(frame_num = 0):
    ...
    frame_suffix_data.timestamp = 762.63  # type: ignore[assignment]
